...
def TrainOnLineGraph():
    ...
    options = model11.get_options()
    
    agent = model22.QAgent(options)
    ...
    checkpoint = tf.train.get_checkpoint_state("../Graph2/184")
    ...
    for i_episode in range(options.MAX_EPISODE):   
        ...
        while not done:
            ...
            #reward=-Observation_Data[7]/100-Observation_Data[2]-Observation_Data[3]
            
            #reward=-abs(Observation_Data[0]-80)
            
             #np.log
            #reward=np.log(Observation_Data[6]/Observation_Data[7])*80-Observation_Data[2]*Observation_Data[2]*2-Observation_Data[3]*Observation_Data[3]
            
            #if (local_step>=199):
            #    reward-=2000
            
            reward=np.log(Observation_Data[6]/Observation_Data[7])*50-Observation_Data[2]*Observation_Data[2]*2-Observation_Data[3]*Observation_Data[3]
            
            if Last_Obs[0]==1 and (action1==4 or action1==5 or action1==6 or action1==7):
                reward-=1000
                
            if Last_Obs[0]==300 and (action1==1 or action1==2 or action1==3 or action1==4):
                reward-=1000
            ...
            reward1=np.log(Observation_Data[6]/Observation_Data[7])*50
            reward2=-Observation_Data[2]*Observation_Data[2]*2
            reward3=-Observation_Data[3]*Observation_Data[3]
            reward4=reward
            ...
            if global_step >= options.MAX_EXPERIENCE:
                ...
                #rand_indexs = np.arange(0,options.MAX_EXPERIENCE,1)
                #print rand_indexs
        ...
        if (i_episode+1)%10==0:
            os.makedirs('../Graph2/'+str(i_episode+1))
            saver.save(sess, '../Graph2/' + str(i_episode+1) + '/' + 'dqn',global_step = i_episode+1)   
            print("Graph_saved !")
